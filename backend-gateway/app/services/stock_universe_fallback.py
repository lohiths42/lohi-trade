...
_SEED: List[_Sec] = [
    ...
    _Sec(
        4, "INFY", "INE009A01021", "Infosys Ltd", "NSE", "IT/Technology", "IT Services", "large-cap"
    ),
    _Sec(
        5,
        "ICICIBANK",
    ...
    _Sec(
        17, "WIPRO", "INE075A01022", "Wipro Ltd", "NSE", "IT/Technology", "IT Services", "large-cap"
    ),
    _Sec(
        18,
        "SUNPHARMA",
...
class FallbackStockUniverseService:
    ...
    async def search_securities(self, q: str, limit: int = 20) -> list[_Sec]:
        ...
        hits: list[_Sec] = []
        for s in _SEED:
            if needle in s.symbol or needle in s.company_name.upper() or needle == s.isin.upper():
                hits.append(s)
                if len(hits) >= limit:
                    break
        return hits
