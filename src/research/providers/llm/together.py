"""Together LLM adapter (Req 2.4, design §3.1).

Together's chat-completions endpoint
(``https://api.together.xyz/v1/chat/completions``) is OpenAI-wire
...
``src.research.providers.base`` against Together and nothing else. It
is registered lazily by ``registry.py`` via the
``"together": "src.research.providers.llm.together:build"`` entry, so
...
* 401/403 → ``ProviderAuthError(provider="together", model=…,
...
# Together's OpenAI-compatible chat-completions endpoint (design §3.1).
...
class TogetherLLM:
    """Concrete ``LLMProvider`` hitting the Together chat-completions endpoint.
    ...
    def _build_payload(
        ...
        *,
        ...
        minimal payload and let Together fall back to its own defaults
        for unspecified knobs. Unknown keys passed via ``params.extra``
        are forwarded verbatim for provider-specific knobs — Req 2.4
        wants one-file-per-backend so there is no central allow-list
        to maintain.
        """
            ...
            async with client.stream(
                ...
                async for raw_line in response.aiter_lines():
                    ...
                    if not raw_line.startswith("data:"):
                        # Together occasionally emits ``event:`` / ``id:`` frames.
                        continue
...
def build(cfg: dict) -> LLMProvider:
    ...
    return TogetherLLM(api_key=api_key, model=model, base_url=base_url)


__all__ = ["TogetherLLM", "build"]
