"""NVIDIA NIM LLM adapter — default cloud provider (Req 2.4, Req 2.7, Req 2.10, design §3.1).

NVIDIA NIM (``build.nvidia.com``) is the Default_Cloud_Provider for
Lohi-Research: it hosts OpenAI-compatible chat-completion endpoints at
``https://integrate.api.nvidia.com/v1/chat/completions`` and offers a
free tier, which is why Req 2.7 names it as the fall-through default
when no provider is configured.

This module implements the ``LLMProvider`` protocol declared in
``src.research.providers.base`` against that endpoint and nothing else.
It is registered lazily by ``registry.py`` via the
``"nvidia_nim": "src.research.providers.llm.nvidia_nim:build"`` entry,
...
# OpenAI-compatible NVIDIA NIM cloud endpoint (design §3.1, Req 2.7).
...
class NvidiaNimLLM:
    """Concrete ``LLMProvider`` hitting the NVIDIA NIM cloud endpoint.
    ...
    def _build_payload(
        ...
        *,
        ...
        minimal payload and let NIM fall back to its own defaults for
        unspecified knobs. Unknown keys passed via ``params.extra`` are
        forwarded verbatim for provider-specific knobs (e.g.
        ``presence_penalty``) — Req 2.4 wants one-file-per-backend so
        there is no central allow-list to maintain.
        """
            ...
            async with client.stream(
                ...
                async for raw_line in response.aiter_lines():
                    ...
                    if not raw_line.startswith("data:"):
                        # NIM occasionally emits ``event:`` / ``id:`` frames.
                        continue
...
def build(cfg: dict) -> LLMProvider:
    ...
    for pointing at a self-hosted NIM microservice or a staging URL).
    ...
    try:
        ...
        missing = exc.args[0]
        raise KeyError(
            f"nvidia_nim provider config is missing required key {missing!r}; "
            "expected 'model' and 'api_key' (see design §7.1).",
        ) from exc

    base_url = cfg.get("base_url") or _DEFAULT_BASE_URL
    return NvidiaNimLLM(api_key=api_key, model=model, base_url=base_url)


__all__ = ["NvidiaNimLLM", "build"]
