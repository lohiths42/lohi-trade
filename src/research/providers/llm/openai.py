"""OpenAI LLM adapter (Req 2.4, Req 2.10, design §3.1).

OpenAI's chat-completions endpoint
(``https://api.openai.com/v1/chat/completions``) is the reference
implementation of the OpenAI-compatible wire format that NVIDIA NIM
(``src.research.providers.llm.nvidia_nim``) and several other backends
also speak. The two adapters are therefore structurally identical —
same request payload, same SSE parsing, same auth-error mapping — and
differ only in the default base URL and the ``provider`` tag stamped
onto returned ``Completion`` / ``CompletionChunk`` envelopes.
...
``"openai": "src.research.providers.llm.openai:build"`` entry, so
...
# OpenAI chat-completions endpoint (design §3.1).
...
class OpenAILLM:
    """Concrete ``LLMProvider`` hitting the OpenAI chat-completions endpoint.
    ...
    def _build_payload(
        ...
        *,
        ...
        minimal payload and let OpenAI fall back to its own defaults for
            ...
            async with client.stream(
                ...
                async for raw_line in response.aiter_lines():
                    ...
                    if not raw_line.startswith("data:"):
                        # OpenAI occasionally emits ``event:`` / ``id:`` frames.
                        continue
