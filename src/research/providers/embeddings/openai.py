"""OpenAI embeddings adapter (Req 2.5, design §3.1).

OpenAI's embeddings endpoint (``https://api.openai.com/v1/embeddings``)
is the reference implementation of the OpenAI-compatible wire format
that NVIDIA NIM embeddings
(``src.research.providers.embeddings.nvidia_nim``) and several other
backends also speak. The two adapters are therefore structurally
similar — same request body, same response shape, same auth-error
mapping — and differ only in the default base URL, the absence of NIM's
``input_type`` field, and the ``provider`` tag carried in the raised
``ProviderAuthError``.

This module implements the ``EmbeddingsProvider`` protocol declared in
``src.research.providers.base`` against OpenAI and nothing else. It is
registered lazily by ``registry.py`` via the
``"openai": "src.research.providers.embeddings.openai:build"`` entry,
...
* ``embed`` — POST ``/v1/embeddings`` with ``{"input": texts,
  "model": …, "encoding_format": "float"}``. Returns one
  ``list[float]`` per input, re-ordered by the ``index`` field from the
  response so the output is aligned with ``texts`` even when the server
...
* ``dim`` — returns ``cfg["dim"]`` when the operator pinned it up-
  ...
  coroutine).
* 401/403 → ``ProviderAuthError(provider="openai", model=…,
...
# OpenAI embeddings endpoint (design §3.1).
_DEFAULT_BASE_URL = "https://api.openai.com/v1"

# Embeddings are typically sub-second; 30s is generous enough to cover
# batched calls but short enough to fail fast when the upstream is
# unreachable (design §3.4).
...
class OpenAIEmbeddings:
    """Concrete ``EmbeddingsProvider`` hitting the OpenAI embeddings endpoint.
    ...
    async def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        order matches ``texts`` even if the server returns the ``data``
        array in a different order; OpenAI documents this behaviour
        explicitly and every OpenAI-compatible backend honours it.
        """
    ...
    def dim(self) -> int:
        ...
        if self._dim is None:
            raise RuntimeError(
                "openai embeddings: dim is not yet known. Either set "
...
def build(cfg: dict[str, Any]) -> EmbeddingsProvider:
    ...
    ``config/settings.yaml`` when ``provider == "openai"``; it is
    forwarded verbatim by the registry.

    Required keys:
      * ``api_key`` — OpenAI bearer token (environment-interpolated).
      * ``model``   — embeddings model id, e.g. ``"text-embedding-3-small"``.

    Optional keys:
      * ``base_url``   — override for OpenAI-compatible gateways (e.g.
                         Azure, a staging URL).
    ...
    try:
        ...
        missing = exc.args[0]
        raise KeyError(
            f"openai embeddings provider config is missing required "
...
__all__ = ["OpenAIEmbeddings", "build"]
